...
## Conclusion
Implementing monitoring is easy, however implementing it correctly can be challenging, and depending on the project might not be worth it at all. The developers that chooses not to implement monitoring, does so because of the many challenges a poorly implemented monitoring system can have, we as student were able to implement it easily, but it must be stressed that such implementation lacks proper security of the metrics, although some developers just seem to feel monitoring can actively harm the system, but from research this seems to be mostly a result of bad experience from poor implementation.  
When choosing when to implement monitoring, it's important to take in scale and importances of data, sometimes it cost too much to implement it when the return of investment is small or negativ, choose carefully when you do, it's a very powerful tool that can either make or break your product.  
